...
{ 
     ...
     ofstream outfile("./dbpedia-giraph-temp.txt");
...
for(int i  =1 ; i <= cnt; i++)
{
   ...
   outfile << i  << "," ; //<< id_label[id] << ";" << "1;3;4;10;15!";// << " " << info[i].size() ;
  for(iter = info[i].begin(); iter != info[i].end(); iter++)
  {
   outfile  <<  (*iter).id << ",";// << (*iter).edge << "\t"  ;//<< " " << id_label[*iter];
  }
  outfile << "!"  << id_label[i] << ";" << "1,1,2,3,4;2,2,3;3,3,4;4,4";
//outfile<<"!"<<id_label[i]<<";"<<"1,3,2,3;2,8,3;3,2,4;4,4,5;5,5,3";  
outfile << endl;
}
    return 0;
